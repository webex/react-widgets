...
| Generator runtime | codex-desktop |
| Validator runtime | pending Codex Session B (different-runtime from `codex-desktop`) |
...
| Spec-validator (Axis A + B) | **not-run** | Reset after 0.2.2 upgrade; Codex Session B must re-validate at HEAD |

## akulakum PR #1468 review closure (2026-09-03)

| Thread | Fix |
|---|---|
| Validation provenance / SHA drift | Cleared `67390b52` / `2004c129` claims; manifest and module Validation status reset to `not-run` pending Session B |
