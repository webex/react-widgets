...
function handleIncomingCalls(sparkInstance, dispatch) {
    ...
    return incomingCall.acknowledge()
      .then(() => {
        const removeIncoming = () => {
          incomingCall.off();
          return dispatch(removeCall(incomingCall));
        };
...
export function listenForCalls(sparkInstance) {
  return (dispatch) => {
    ...
    return Promise.all([
      handleIncomingCalls(sparkInstance, dispatch),
...
export function listenForIncomingCalls(sparkInstance) {
  return (dispatch) => {
    dispatch(updateStatus({isListeningToIncoming: true}));
    return handleIncomingCalls(sparkInstance, dispatch);
  };
}
