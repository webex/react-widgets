...
describe(`redux-module-media actions `, () => {
  beforeEach(() => {
    ...
    mockSpark = {
      phone: {
        ...
        dial: jest.fn(() => Promise.resolve(call)),
        on: jest.fn((eventName, callback) => Promise.resolve({eventName, callback})),
  ...
  it(`has exported actions`, () => {
    ...
    expect(actions.callUser).toBeDefined();
  ...
  describe(`#updateWebRTCSupport`, () => {
    it(`can update WebRTCSupport status`, () => {
      store.dispatch(actions.updateWebRTCSupport(true));
  ...
  describe(`#callUser`, () => {
    it(`can successfully call a user with email`, () => {
      const options = {
        userIdOrEmail: `test@ciscospark.com`,
        locusUrl: `https://locusUrl`
      };
      store.dispatch(actions.callUser(options, mockSpark))
        .then(() => {
          expect(mockSpark.phone.createLocalMediaStream).toHaveBeenCalled();
  ...
  describe(`#listenForIncomingCalls`, () => {
    it(`can successfully attach call:incoming listener`, () =>
      store.dispatch(actions.listenForIncomingCalls(mockSpark, `https://locusUrl`))
        .then(() => {
        ...
        .then(({eventName, callback}) => {
          ...
          return callback(call)
            .then(() => {
              ...
              expect(call.once.mock.calls[0][0]).toBe(`disconnected`);
              expect(call.once.mock.calls[1][0]).toBe(`connected`);
              expect(store.getActions()).toMatchSnapshot();
            });
        })
    );
