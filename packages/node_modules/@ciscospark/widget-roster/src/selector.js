...
const getSearchResults = createSelector(
  ...
  (search, widgetRoster) => {
    const searchTerm = widgetRoster.getIn([`widgetState`, `searchTerm`]);
    if (searchTerm && search.hasIn([`searchResults`, searchTerm])) {
      return search.getIn([`searchResults`, searchTerm]).toJS();
    }
    return undefined;
  }
);
...
export const getRosterWidgetProps = createSelector(
  ...
  (sparkInstance, conversation, user, avatars, searchResults, widgetRoster) => {
    ...
    const isOneOnOne = conversation.get(`isOneOnOne`);
    const isLocked = conversation.get(`isLocked`);
    const isModerator = conversation.get(`isModerator`);
    const canEditRoster = !isOneOnOne && (isLocked && isModerator || !isLocked);

    propsObject.canEditRoster = canEditRoster;
    propsObject.currentView = widgetRoster.getIn([`widgetState`, `currentView`]);
    propsObject.searchTerm = widgetRoster.getIn([`widgetState`, `searchTerm`]);
