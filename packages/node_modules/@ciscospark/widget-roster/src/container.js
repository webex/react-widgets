...
export class RosterWidget extends Component {
  ...
  render() {
    ...
    if (!this.props.conversation.has(`participants`)) {
      ...
      const participants = this.props.conversation.get(`participants`).toJS();
      const currentUserId = this.props.user.get(`currentUser`).id;
      ...
      mainArea = (
        <div>
          <div><ParticipantList participants={currentUserParticipant} /></div>
          <div className={classNames(`ciscospark-roster-participants-separator`, styles.participantsSeparator)}>{`Participants`}</div>
          <div><ParticipantList participants={otherParticipants} /></div>
        </div>
      );
