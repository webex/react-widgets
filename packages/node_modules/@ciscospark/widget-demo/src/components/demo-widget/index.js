...
class DemoWidget extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      spaceRunning: false,
      spaceTypeFilter: SPACE_TYPE_ALL,
      spaceWidgetProps: {},
      stickyMode: false
    };
  }
  ...
  handleOpenRecentsWidget(e) {
    ...
    cookies.set('accessToken', this.state.accessToken);
    cookies.set('accessTokenType', this.state.accessTokenType);
    cookies.set('extendedLoad', this.state.extendedLoad);
    ...
    const widgetOptions = {
      ...
      extendedLoad: this.state.extendedLoad,
      spaceLoadCount: this.state.spaceLoadCount
    };

    if (this.state.accessTokenType === 'JWT') {
      widgetOptions.guestToken = this.state.accessToken;
      widgetOptions.spaceTypeFilter = this.state.spaceTypeFilter;
      widgetOptions.enableSpaceListFilter = this.state.enableSpaceListFilter;
    }
    else {
  ...
  @autobind
  handleSpaceTypeFilterChange(value) {
    this.setState({spaceTypeFilter: value});
  }

  @autobind
  handleRecentSpaceLoadCountChange(e) {
    this.setState({spaceLoadCount: e.target.value});
  }
  ...
  @autobind
  handleRecentExtendedLoadToggle() {
    this.setState((state) => (
      {extendedLoad: !state.extendedLoad}
    ));
  }
          ...
          <div className={styles.section}>
            <div>
              ...
              <Checkbox
                ...
                disabled={this.state.recentsRunning}
              />
              <h3>Space Type Filter</h3>
              <RadioGroup
                ariaLabel="Space Type Filter"
                name="spaceTypeFilter"
                onChange={(e) => this.handleSpaceTypeFilterChange(e)}
                values={[this.state.spaceTypeFilter]}
              >
                <Radio
                  ariaLabel="Space Type All Filter"
                  htmlId="spaceTypeAllFilter"
                  label="All"
                  value={SPACE_TYPE_ALL}
                  disabled={this.state.recentsRunning}
                />
                <Radio
                  ariaLabel="Space Type Group Filter"
                  htmlId="spaceTypeGroupFilter"
                  label="Group"
                  value={SPACE_TYPE_GROUP}
                  disabled={this.state.recentsRunning}
                />
                <Radio
                  ariaLabel="Space Type One-to-One Filter"
                  htmlId="spaceTypeOnetoOneFilter"
                  label="One on One"
                  value={SPACE_TYPE_ONE_ON_ONE}
                  disabled={this.state.recentsRunning}
                />
              </RadioGroup>
              <h3>Extended Load</h3>
              <Checkbox
                checked={this.state.extendedLoad}
                htmlId="extendedLoad"
                label="Enable Extended Space Loading"
                onChange={this.handleRecentExtendedLoadToggle}
                value="Enable Extended Space Loading"
                disabled={this.state.recentsRunning}
              />
              <Button
