...
export class MessageWidget extends Component {
  ...
  componentWillUpdate(nextProps) {
    ...
    if (!nextProps.features.hasIn([`items`, `js-widgets-mentions`])) {
      props.getFeature(`developer`, `js-widgets-mentions`, nextProps.sparkInstance);
    }
  }
  ...
  setupConversationActions(conversation, nextProps) {
    const {flags, sparkInstance, space, widgetMessage} = nextProps;
    const {props} = this;
  ...
  listenToNewActivity(space, sparkInstance) {
    ...
    const currentUserId = props.user.get(`currentUser`).id;
  ...
  listenToBufferState(conversationId, sparkInstance) {
    ...
    sparkInstance.internal.mercury.on(`event:mercury.buffer_state`, (event) => {
      ...
      if (_.has(event, `data.bufferState.conversation`) && event.data.bufferState.conversation === `UNKNOWN`) {
        ...
        props.loadMissingActivities(conversation.get(`id`), sinceDate, sparkInstance);
      }
    });
  }
  ...
  render() {
    ...
    if (conversation && conversation.get(`id`)) {
      ...
      const showMentions = features.getIn([`items`, `js-widgets-mentions`]);
