...
export class MessageWidget extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (activityList) {
      const activities = props.conversation.activities;
      const prevActivities = prevProps.conversation.activities;
      ...
      this.updateScroll(activities, prevActivities, prevProps.widget.scrollTop);
  ...
  setupConversationActions(conversation, nextProps) {
    ...
    if (!conversation.mercuryState.isListening) {
      this.listenToTypingEvents(conversation.id, spark);
      this.listenToNewActivity(conversation.id, spark);
    }
  ...
  listenToNewActivity(conversationId, spark) {
    ...
    spark.mercury.on(`event:conversation.activity`, (event) => {
      ...
      const isVisibleContent = VISIBLE_ACTIVITY_VERBS.indexOf(activity.verb) !== -1
        && VISIBLE_ACTIVITY_TYPES.indexOf(activity.object.objectType) !== -1;
      ...
      if (activity.target.id === conversationId) {
        if (isVisibleContent) {
          ...
          handleEvent(events.EVENT_MESSAGES_UNREAD, activity);
          handleEvent(events.EVENT_MESSAGES_CREATED, activity);
        }
  ...
  handleConfirmActivityDelete() {
    ...
    const activityId = widget.get(`deletingActivityId`);
    const activity = conversation.activities.find((act) => act.id === activityId);
    if (activity) {
  ...
  handleScroll() {
    ...
    else if (!widget.get(`showScrollToBottomButton`)) {
      props.showScrollToBottomButton(true);
    }

    if (activityList.isScrolledToTop() && conversation.activities.first().verb !== `create`) {
  ...
  render() {
    ...
    const scrollToBottomLabel = widget.get(`hasNewMessage`) ? formatMessage(messages.newMessagesMessage) : ``;
        ...
        >
          <div className={classNames(`activity-list-wrapper`, styles.activityListWrapper)}>
            ...
            >
              ...
              />
              <div className={classNames(`indicators`, styles.indicators)}><ReadReceipts /></div>
            </ScrollingActivity>
            {
              widget.get(`showScrollToBottomButton`) &&
          ...
          <div className={classNames(`message-composer-wrapper`, styles.messageComposerWrapper)}>
            <MessageComposer
              ...
              placeholder={formatMessage(messages.messageComposerPlaceholder, {displayName: toUser.displayName})}
              spark={spark}
            />
          </div>
          {
            props.widget.get(`showAlertModal`) &&
            <ConfirmationModal
