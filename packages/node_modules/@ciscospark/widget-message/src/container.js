...
import {
  EVENT_MESSAGES_CREATED,
  EVENT_MESSAGES_UNREAD
} from './constants';
...
class MessageWidget extends Component {
    ...
    spark.mercury.on(`event:conversation.activity`, (event) => {
      ...
      if (activity.target.id === conversationId) {
        if (isVisibleContent) {
          ...
          handleEvent(EVENT_MESSAGES_UNREAD, activity);
          handleEvent(EVENT_MESSAGES_CREATED, activity);
        }
  ...
  /**
   * Empty Event handler in case one isn't provided
   * @param {string} name
   * @param {object} data
   * @returns {undfined}
   */
