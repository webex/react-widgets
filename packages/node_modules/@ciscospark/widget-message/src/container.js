...
import {fetchAvatarForUserId} from '@ciscospark/redux-module-user';
import {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (activityList) {
      ...
      if (lastActivityFromPrev
        ...
        if (props.conversation.participants.items[0].id !== lastActivityFromThis.actor.id) {
  ...
  getAvatarsFromConversation(conversation) {
    ...
    const userIds = _.uniq(participants.map((participant) => participant.id))
      .filter((userId) => {
        ...
        const fetched = avatar.items.hasOwnProperty(userId);
        const fetching = avatar.avatarsInFlight.indexOf(userId) !== -1;
        ...
        >
          <div className={classNames(`activity-list-wrapper`, styles.activityListWrapper)}>
            ...
            </ScrollingActivity>
            {
              ...
              <ScrollToBottomButton
                label={formatMessage(messages.newMessagesMessage)}
          ...
          <div className={classNames(`message-composer-wrapper`, styles.messageComposerWrapper)}>
            <MessageComposer
              ...
              placeholder={formatMessage(messages.messageComposerPlaceholder, toUser.displayName)}
              spark={spark}
            />
          </div>
          {
            props.widget.showAlertModal &&
            <ConfirmationModal
