...
export class MessageWidget extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (activityList) {
      ...
      const prevActivities = prevProps.activities;
      const firstActivity = props.firstActivity;
      const previousFirstActivity = props.firstActivity;
  ...
  shouldScrollToBottom(props, prevProps) {
    ...
    const lastActivityFromThis = props.lastActivity;
    // If new activity comes in
    if (lastActivityFromPrev && lastActivityFromThis && props.activityCount !== prevProps.activityCount && lastActivityFromPrev.id !== lastActivityFromThis.id) {
  ...
  setupConversationActions(conversation, nextProps) {
    const {flags, sparkInstance, space, widgetMessage, conversationId} = nextProps;
    const {props} = this;
    ...
    if (!widgetMessage.get(`isListeningToBufferState`)) {
      this.listenToBufferState(conversationId, sparkInstance);
    }
  }
   ...
   * @returns {null}
   */
  getAvatarsFromParticipants(props) {
    const {
      participants,
      sparkInstance
    } = props;
