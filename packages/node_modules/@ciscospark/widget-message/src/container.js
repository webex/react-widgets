...
export class MessageWidget extends Component {
  ...
  establishConversation(props) {
    ...
    if (spark && authenticated) {
      ...
      if (registered && !conversation.get(`id`) && !conversation.get(`isFetching`)) {
  ...
  setupConversationActions(conversation, nextProps) {
    ...
    if (!conversation.get(`mercuryState`).isListening) {
    ...
    if (conversation.get(`participants`).length && conversation.get(`participants`).length !== prevConversation.get(`participants`).length) {
  ...
  handleFileDrop(acceptedFiles) {
    ...
    props.addFiles(conversation.toJS(), activity, files, spark);
  }
  ...
  handleScroll() {
    ...
    if (activityList.isScrolledToBottom()) {
      ...
      if (conversation.get(`lastAcknowledgedActivityId`) !== lastActivity.id) {
        ...
        props.acknowledgeActivityOnServer(conversation.toJS(), lastActivity, spark);
      }
    }
  ...
  render() {
    ...
    if (conversation && conversation.get(`id`)) {
      const isLoadingHistoryUp = conversation.get(`isLoadingHistoryUp`);
