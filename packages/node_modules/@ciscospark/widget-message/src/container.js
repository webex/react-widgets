...
import {constructActivityEventData} from '@ciscospark/react-component-utils';
...
export class MessageWidget extends Component {
  ...
  componentDidMount() {
    this.eventNames = Object.assign({}, events, this.props.eventNames);
  ...
  setupConversationActions(conversation, nextProps) {
    const {flags, sparkInstance} = nextProps;
    const {props} = this;
    ...
    if (!conversation.getIn([`status`, `isListening`])) {
      ...
      this.listenToNewActivity(id, sparkInstance);
    }
  ...
  listenToTypingEvents(conversationId, sparkInstance) {
    ...
    sparkInstance.mercury.on(`event:status.start_typing`, (event) => {
      if (event.data.conversationId === conversationId) {
        props.setTyping(event.data.actor.id, true);
      }
    });

    sparkInstance.mercury.on(`event:status.stop_typing`, (event) => {
      if (event.data.conversationId === conversationId) {
  ...
  listenToNewActivity(conversationId, sparkInstance) {
    ...
    sparkInstance.mercury.on(`event:conversation.activity`, (event) => {
      ...
      if (activity.target.id === conversationId) {
        if (activity.object.objectType === `activity`) {
          ...
          if (activity.actor.id !== props.user.get(`currentUser`).id) {
            props.updateHasNewMessage(true);
            handleEvent(this.eventNames.SPACES_UNREAD, eventActivity);
          }
          handleEvent(this.eventNames.MESSAGES_CREATED, eventActivity);
        }
      }
    });
  }
  ...
  listenToBufferState(conversationId, sparkInstance) {
    sparkInstance.mercury.on(`event:mercury.buffer_state`, (event) => {
      const {props} = this;
      const {
        conversation
      } = props;
  ...
  handleScroll() {
    ...
    if (activityList.isScrolledToBottom()) {
      ...
      if (conversation.get(`lastAcknowledgedActivityId`) !== lastActivity.id) {
        this.handleEvent(this.eventNames.SPACES_READ, constructActivityEventData(lastActivity));
  ...
  render() {
    ...
    if (conversation && conversation.get(`id`)) {
      ...
      const displayName = conversation.get(`displayName`) || toUser.get(`displayName`);
