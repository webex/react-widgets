...
import {hasAdaptiveCard, FEATURES_WIDGET_ADAPTIVE_CARD_ON, ACTIVITY_CARDS} from '@ciscospark/react-component-utils';
...
function ActivityItem(props) {
  ...
  if (Object.prototype.hasOwnProperty.call(activity, ACTIVITY_CARDS)) {
    shouldRenderAdaptiveCard = hasAdaptiveCard(activity.cards, sparkInstance);
  }
  if (verb === POST_VERB) {
    itemComponent = (shouldRenderAdaptiveCard && adaptiveCardFeatureState === FEATURES_WIDGET_ADAPTIVE_CARD_ON)
      ? <AdaptiveCard cards={activity.cards} displayName={activity.displayName} /> : (<ActivityPost
      ...
      itemComponent = (
        <ActivityShareFiles
          renderAdaptiveCard={(shouldRenderAdaptiveCard && adaptiveCardFeatureState === FEATURES_WIDGET_ADAPTIVE_CARD_ON
            ) ? shouldRenderAdaptiveCard : !shouldRenderAdaptiveCard}
  ...
  return (
    <div className={classNames('ciscospark-activity-item-container', styles.activityItemContainer, isAdditional ? styles.additional : '')}>
      {itemComponent}
    </div>
  );
}
