...
export default function reducer(state = initialState, action) { // eslint-disable-line complexity
  ...
  case STORE_AVATAR_BY_SPACE: {
    const hasAvatar = state.getIn([`items`, action.payload.spaceId, `avatar`]);
    if (hasAvatar) {
      return state.mergeDeepIn([`items`, action.payload.spaceId, `avatar`], action.payload.avatar);
    }
    return state.setIn([`items`, action.payload.spaceId, `avatar`], action.payload.avatar);
  }
