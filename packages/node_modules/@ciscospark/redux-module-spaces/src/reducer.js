...
export default function reducer(state = initialState, action) { // eslint-disable-line complexity
  ...
  case STORE_SPACES: {
    const newSpaces = new OrderedMap(action.payload.spaces.map((s) => [s.id, fromJS(s)]));
    const spaces = state.get(`items`).mergeDeep(newSpaces).sortBy(sortByNewest);
  ...
  case UPDATE_SPACE_READ: {
    ...
    let spaces = state.get(`items`);
    if (spaces.get(spaceId)) {
      spaces = spaces.setIn([spaceId, `lastSeenActivityDate`], lastSeenDate);
    }
    return state.set(`items`, spaces.sortBy(sortByNewest));
  }
  ...
  case HIDE_SPACE:
    // We're just deleting the room from store for now
    // If new activity comes in for this space, we'll fetch it again
    return state.deleteIn([`items`, action.payload.spaceId]);
