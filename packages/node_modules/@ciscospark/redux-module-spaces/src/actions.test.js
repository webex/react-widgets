...
let activity, avatar, file, mockSpark, person, space, spaces, store, team, user1, user2;
...
describe('redux-module-space actions ', () => {
  ...
  it('has exported actions', () => {
    expect(actions.fetchSpaces).toBeDefined();
    expect(actions.getSpaces).toBeDefined();
    ...
    expect(actions.updateSpaceRead).toBeDefined();
  });
  ...
  describe('#updateSpaceRead', () => {
    it('properly updates lastSeenActivityDate for a space', () => {
      store.dispatch(actions.updateSpaceRead('conversationId', '2017-06-07T15:13:34.505Z'));
      expect(store.getActions()).toMatchSnapshot();
    });
  });
});
