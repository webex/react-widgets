...
export const ADD_SPACE_ERROR = 'spaces/ADD_SPACE_ERROR';
...
export function addSpaceError(id, error) {
  return {
    type: ADD_SPACE_ERROR,
    payload: {
      id,
      error
    }
...
function decryptSpace(space) {
  ...
  return new Error('Space cannot be decrypted');
}
      ...
      const spaces = items.map((space) => {
        ...
        if (deferDecrypt && space.decrypt) {
          decryptSpace(space)
            .then((decryptedSpace) => {
              if (decryptedSpace) {
                return dispatch(processSpace(decryptedSpace, {isDecrypting: false}));
              }
              return new Error('Space was not decrypted correctly');
            });
        }
        return s;
      });
...
export function fetchSpace(sparkInstance, spaceId) {
  return (dispatch) => {
    ...
    return sparkInstance.internal.conversation.get({
      ...
      .then((space) => dispatch(processSpace(space)));
  };
}
