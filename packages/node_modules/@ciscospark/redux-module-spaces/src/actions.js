...
export const STORE_AVATAR_BY_SPACE = `spaces/STORE_AVATAR_BY_SPACE`;
...
function storeAvatarBySpace(spaceId, avatar) {
  return {
    type: STORE_AVATAR_BY_SPACE,
    payload: {
      spaceId,
      avatar
    }
...
export function initialFetchSpaces(sparkInstance) {
  return (dispatch) => {
    ...
    .then(({spaces, teams}) => {
      spaces.forEach((s) => dispatch(fetchSpaceAvatar(sparkInstance, s)));
      return dispatch(fetchSpaces(sparkInstance, {
...
export function fetchSpaceAvatar(sparkInstance, space) {
  return (dispatch) => {
    if (!space.avatar || !space.avatar.url) {
      const currentUserId = sparkInstance.device.userId;
      if (space.type === `direct`) {
        ...
        return sparkInstance.people.get(otherUserId)
          .then((person) => {
            dispatch(storeAvatarBySpace(space.id, {url: person.avatar}))
          });
      }
      else if (space.avatar) {
        return sparkInstance.conversation.download(space.avatar.files.items[0])
          .then((file) => {
            const {blob, objectUrl} = bufferToBlob(file);
            dispatch(storeAvatarBySpace(space.id, {url: objectUrl, blob}));
          });
      }
    }
...
function fetchSpaces(sparkInstance, options = {}, teams = {}) {
  ...
  return (dispatch) => sparkInstance.conversation
