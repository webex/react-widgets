...
export function fetchSpaces(sparkInstance) {
  ...
  return (dispatch) => {
    ...
    // fetch first 50 from the last week of conversations
    return sparkInstance.conversation.list(Object.assign({}, listOptions, {
      sinceDate: moment().subtract(`weeks`, 1).valueOf(),
      conversationsLimit: 50
    })).then((limitedItems) => {
      dispatch(storeSpaces(limitedItems));
      // fetch all conversations
      return sparkInstance.conversation.list(Object.assign({}, listOptions, {
        conversationsLimit: 1000
      })).then((allItems) => {
        dispatch(updateSpacesStatus({isFetching: false}));
        dispatch(storeSpaces(allItems));
      });
    });
  };
}
