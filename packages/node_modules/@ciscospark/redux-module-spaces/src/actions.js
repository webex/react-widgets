...
export const HIDE_SPACE = `spaces/HIDE_SPACE`;
...
export function initialFetchSpaces(sparkInstance) {
  return (dispatch) => {
    ...
    return dispatch(fetchSpaces(sparkInstance, {
      sinceDate: moment().subtract(`weeks`, 4).valueOf(),
...
export function hideSpace(spaceId) {
  return {
    type: HIDE_SPACE,
    payload: {
      spaceId
...
function constructSpace(space, teams) {
  const s = {
    ...
    hidden: space.tags.includes(TAG_HIDDEN),
