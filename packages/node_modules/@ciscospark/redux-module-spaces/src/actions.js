 ...
 * @param {Integer} count number of spaces to fetch
 * @param {Object} teams all teams
 * @returns {Function} thunk
 */

export function fetchSpaces(sparkInstance, count = 15, teams = {}) {
  return (dispatch) => {
    dispatch(updateSpacesStatus({isFetching: true}));
    return dispatch(getSpaces(sparkInstance, {
      conversationsLimit: count
    }, teams))
    .then(({spaces}) => {
...
export function getSpaces(sparkInstance, options = {}, teams = {}) {
  const listOptions = {
    ackFilter: `noack`,
    ...
    .then((items) => {
      ...
      items.forEach((space) => {
        if (!space.tags.includes(TAG_HIDDEN)) {
          spaces[space.id] = constructSpace(space);
        }
      });
