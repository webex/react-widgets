...
Be sure to add the `ciscospark-widget` class to your div to ensure that appropriate styles are applied.

```html
<div id="ciscospark-widget-id" class="ciscospark-widget"/>
<script>
...
> `ciscospark-widget-id` is an arbitrary id to illustrate one way to select the DOM element.
...
The widget grows and shrinks based on the container that you place it in. If you do not provide a container, the widget will fill the space of the closest parent.
