...
import {MercuryComponent} from '@ciscospark/redux-module-mercury';
...
export class MessageMeetWidget extends Component {
  ...
  componentWillMount() {
    ...
    if (spark && sparkState.authenticated) {
    ...
    if (spark && authenticated) {
      ...
      if (registered && connected) {
        if (!media.getIn([`status`, `isListening`]) && conversation.get(`locusUrl`)) {
          nextProps.listenForIncomingCalls(spark, conversation.get(`locusUrl`), onEvent);
        }
  ...
  fetchUsers(props) {
    ...
    if (sparkState.registered && !user.get(`currentUser`) && !user.getIn([`status`, `isFetchingCurrentUser`])) {
      props.fetchCurrentUser(spark);
    }
    if (!user.get(`toUser`) && !user.getIn([`status`, `isFetchingToUser`])) {
      props.fetchToUser({toPersonEmail, toPersonId}, spark);
    }
  }
