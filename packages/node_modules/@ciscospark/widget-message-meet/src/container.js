...
export class MessageMeetWidget extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (spark && connected && authenticated && registered) {
      ...
      if (user.get(`currentUser`) && conversation.locusUrl && !media.getIn([`status`, `isListening`])) {
        nextProps.listenForIncomingCalls(spark, conversation.locusUrl);
      }
      // When receivng an incoming call, switch to meet
      if (media.getIn([`status`, `isIncoming`]) && widgetMessageMeet.get(`activityType`) !== ACTIVITY_TYPE_MEET) {
...
const injectedPropTypes = {
  ...
  fetchToUser: PropTypes.func,
  listenForIncomingCalls: PropTypes.func,
