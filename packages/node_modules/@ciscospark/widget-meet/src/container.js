...
export class MeetWidget extends Component {
  ...
  setup(props) {
    ...
    if (incomingCall && !this.props.media.getIn([`incomingCall`, `call`])) {
      this.incomingCallNotification(media.getIn([`incomingCall`, `call`]));
      // Emit the event
      this.handleEvent(this.eventNames.CALLS_CREATED, constructCallEventData(incomingCall, currentUser, spaceId));
    }

    if (media.getIn([`status`, `isConnected`]) && !this.props.media.getIn([`status`, `isConnected`])) {
      // Call is connected, emit event
      this.handleEvent(this.eventNames.CALLS_CONNECTED, constructCallEventData(media.get(`call`), currentUser, spaceId));
    }
  ...
  handleCall() {
    ...
    const currentUser = users.get(`currentUser`);
    props.callUser({userIdOrEmail: toPersonId || toPersonEmail}, sparkInstance)
      .then((call) => {
        // Emit Event

        this.handleEvent(this.eventNames.CALLS_CREATED, constructCallEventData(call, currentUser, conversation.get(`id`)));
      });
  }
  ...
  handleHangup() {
    ...
    const call = media.get(`call`);
    const currentUser = users.get(`currentUser`);
    props.hangupCall(call).then(() => {
      this.handleEvent(this.eventNames.CALLS_DISCONNECTED, constructCallEventData(call, currentUser, conversation.get(`id`)));
    });
  }
