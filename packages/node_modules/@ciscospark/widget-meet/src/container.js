...
export class MeetWidget extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (activeCall && !prevActiveCall) {
      ...
      if (activeCall.getIn([`callState`, `direction`]) === `in`) {
        this.incomingCallNotification(activeCall);
      }
      // Emit the event
      this.handleEvent(
    ...
    if (activeCall && activeCall.getIn([`callState`, `connected`])
      && !prevActiveCall.getIn([`callState`, `connected`])) {
  ...
  render() {
    ...
    if (isWebRTCSupported && (avatarImage || displayName)) {
      ...
      if (call) {
        // Is the call active?
        const direction = call.getIn([`callState`, `direction`]);
        const isConnected = call.getIn([`callState`, `connected`]);
        const isRinging = call.getIn([`callState`, `ringing`]);
        const isInitiated = call.getIn([`callState`, `initiated`]);

        if (isConnected || direction === `out`) {
          const {
            handleHangup,
          ...
          return (
            <div className={classNames(styles.meetWidgetContainer, `meet-widget-container`)}>
              ...
              />
              <Ringtone play={isRinging} type={RINGTONE_TYPE_RINGBACK} />
            </div>
          );
        }

        // Is this an incoming call?
        else if (direction === `in` && isInitiated) {
          const {
            handleAnswer,
            handleDecline
          } = this;
          ...
          return (
            <div className={classNames(styles.meetWidgetContainer, `meet-widget-container`)}>
              ...
              />
              <Ringtone play={true} type={RINGTONE_TYPE_INCOMING} />
            </div>
          );
        }
      }
