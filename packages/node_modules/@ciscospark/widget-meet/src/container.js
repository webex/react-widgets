...
export class MeetWidget extends Component {
  ...
  componentDidMount() {
    ...
    const isIncomingCall = media.getIn([`status`, `isIncoming`]);
    if (isIncomingCall) {
      const incomingCall = media.getIn([`incomingCall`, `call`]);
      props.createNotification(incomingCall.locusUrl, NOTIFICATION_TYPE_CALL);
    }
  }

  componentWillReceiveProps(nextProps) {
    const isIncomingCall = nextProps.media.getIn([`status`, `isIncoming`]);
    const wasIncomingCall = this.props.media.getIn([`status`, `isIncoming`]);
    if (isIncomingCall && !wasIncomingCall) {
      const incomingCall = nextProps.media.getIn([`incomingCall`, `call`]);
      nextProps.createNotification(incomingCall.locusUrl, NOTIFICATION_TYPE_CALL);
    }
  }
...
const injectedPropTypes = {
  ...
  acceptIncomingCall: PropTypes.func,
  callUser: PropTypes.func,
  createNotification: PropTypes.func,
  declineIncomingCall: PropTypes.func,
