...
export class MeetWidget extends Component {
  ...
  componentDidMount() {
    ...
    this.eventNames = Object.assign({}, events, eventNames);
  ...
  setup(props) {
    ...
    if (incomingCall && !this.props.media.getIn([`incomingCall`, `call`])) {
      ...
      // Emit the event
      this.handleEvent(this.eventNames.CALLS_CREATED, {
    ...
    if (media.getIn([`status`, `isConnected`]) && !this.props.media.getIn([`status`, `isConnected`])) {
      ...
      this.handleEvent(this.eventNames.CALLS_CONNECTED, {
        data: media.get(`call`)
      });
    }
  ...
  handleHangup() {
    ...
    props.hangupCall(call).then(() => {
      this.handleEvent(this.eventNames.CALLS_DISCONNECTED, {
        actor: call.from.person,
        data: call
      });
    });
  }
