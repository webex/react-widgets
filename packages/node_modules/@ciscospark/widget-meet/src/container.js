...
export class MeetWidget extends Component {
  ...
  componentWillReceiveProps(nextProps) { // eslint-disable-line complexity
    if (nextProps.isWebRTCSupported) {
      ...
      if (activeCall && callState.connected && !prevCallState.connected) {
  ...
  handleCall() {
    ...
    const currentUser = users.get(`currentUser`);
    props.placeCall({
      destination: toPersonId || toPersonEmail || toPerson || spaceId,
      locusUrl
    }, sparkInstance)
      .then((call) => {
        // Emit Event
  ...
  @autobind
  handleAnswer() {
    const {call, locusUrl} = this.props;
    const incomingCall = call.get(`instance`);
    this.props.acceptIncomingCall(incomingCall, locusUrl).catch(this.catchCallError);
  }
