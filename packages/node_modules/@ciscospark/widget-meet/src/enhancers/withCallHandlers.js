...
function handleDecline(props) {
  return () => {
    const {call} = props;
    props.declineIncomingCall(call);
  };
}
...
function handleCall(props) {
  return () => {
    ...
    props.placeCall(sparkInstance, {destination, options: callOptions})
      .then(({id}) => {
...
export default compose(
  ...
  lifecycle({
    componentWillReceiveProps(nextProps) {
      ...
      // If start call prop starts as true, start call
      if (
      ...
      // If start call prop changes to true, start call
      if (
        (widgetMeet.toValue || call) && currentUser
