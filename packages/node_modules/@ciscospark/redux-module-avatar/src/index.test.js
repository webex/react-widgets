...
describe(`redux-module-avatar actions `, () => {
  ...
  describe(`#fetchSpaceAvatar`, () => {
    ...
    it(`can fetch avatars for spaces`, () => {
      const store = mockStore(mockState);
      return store.dispatch(actions.fetchSpaceAvatar(space, mockSpark))
        .then(() => {
    ...
    it(`should fetch a user avatar for one on one spaces`, () => {
      const store = mockStore(mockState);
      space.type = `direct`;
      return store.dispatch(actions.fetchSpaceAvatar(space, mockSpark, `abc-123`))
        .then(() => {
          expect(mockSpark.people.get).toHaveBeenCalled();
          expect(store.getActions()).toMatchSnapshot();
        });
    ...
    it(`should throw error for one on one spaces without user id`, () => {
      space.type = `direct`;
      expect(actions.fetchSpaceAvatar(space, mockSpark)).toThrow();
    });
