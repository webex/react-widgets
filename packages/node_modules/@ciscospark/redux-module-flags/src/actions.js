...
export function fetchFlags(sparkInstance) {
  return (dispatch) => {
    ...
    return sparkInstance.internal.flag.list()
      .then((flags) =>
        dispatch(storeFlags(flags.map(constructFlag)))
      )
      .catch((error) =>
        dispatch(storeFlags(null, error))
      );
  };
}
...
export function flagActivity(activity, spark) {
  return (dispatch) => {
    ...
    return spark.internal.flag.create(activity)
      ...
      .catch((error) =>
        dispatch(addFlag(flagObject, error))
      );
  };
}
...
export function removeFlagFromServer(flag, spark) {
  return (dispatch) => {
    const flagObject = constructFlag(flag);
    dispatch(removeFlag(flagObject));
    return spark.internal.flag.delete(flag)
      .catch((error) => {
        dispatch(removeFlag(constructFlag(flagObject), error));
      });
  };
}
