...
export class IncomingCall extends Component {
  ...
  componentDidMount() {
    ...
    spark.phone.on(`call:incoming`, (incomingCall) => {
      ...
      // Create notification of call
      this.props.createNotification(incomingCall.locusUrl, NOTIFICATION_TYPE_CALL);
    });
    this.props.listeningForIncomingCalls();
  }

  componentWillReceiveProps(nextProps) {
    const isRinging = nextProps.media.getIn([`status`, `isRinging`]);
    const wasRinging = this.props.media.getIn([`status`, `isRinging`]);
    // Change in ringing state
    if (isRinging !== wasRinging) {
      const isPlaying = this.ringtone.playing();
      if (isRinging && !isPlaying) {
        this.ringtone.play();
      }
      if (!isRinging && isPlaying) {
        this.ringtone.stop();
  ...
  shouldAcknowledgeCall(incomingCall) {
    ...
    if (media.getIn([`status`, `isDialing`])) {
      return false;
    }
    return true;
  }
