...
export function addFiles(conversation, activity, files, spark) {
  return (dispatch) => {
    ...
    if (!shareActivity) {
      shareActivity = spark.conversation.makeShare(conversation.toJS());
...
export function submitActivity(conversation, activity, user, spark) {
  return (dispatch) => {
    ...
    if (shareActivity && activity.get(`files`).size) {
      ...
      return spark.conversation.share(conversationJS, shareActivity)
        .then(() => {
...
export function setUserTyping(isTyping, conversation, spark) {
  return (dispatch) =>
    spark.conversation.updateTypingStatus(conversation.toJS(), {typing: isTyping})
...
export function retryFailedActivity(failedActivity, spark) {
  return (dispatch) => {
    ...
    if (failedActivity.verb === `share`) {
      return spark.conversation.share(conversation, shareActivity)
