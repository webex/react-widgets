...
export default function ButtonControls(props) {
  ...
  const buttonsRendered = buttons.map((button, index) => {
    const label = showLabels && button.label ? button.label : ``;
    const labelPosition = showLabels ? `bottom` : ``;
    return (
      <div className={classNames(`button-control-item`, styles.controlItem)} key={index}>
        <Button
          {...button}
  ...
  return (
    <div className={classNames(`controls-container`, styles.controlContainer)}>
      {buttonsRendered}
    </div>
  );
}
