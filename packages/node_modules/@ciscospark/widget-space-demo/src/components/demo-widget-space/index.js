...
class DemoWidgetSpace extends Component {
  ...
  generateExampleCode(state) {
    const {accessToken, spaceId, toPersonEmail} = state;
...
<SpaceWidget accessToken="${accessToken}" ${reactToField} />`;
    const inlineCode = `<div data-toggle="ciscospark-space" data-access-token="${accessToken}" ${inlineToField} />`;
    return {
      inlineCode,
      reactCode
    };
