...
export default function registerWidget(name, widgetInit, widgetRemove, force = false) {
  ...
  if (typeof window.ciscospark.widgetFn[`${name}Widget`] === 'function' && !force) {
    ...
    window.ciscospark.widgetFn[`${name}Widget`] = function initWidget(options) {
      ...
      // Add version number to widget
      this.version = process.env.REACT_CISCOSPARK_VERSION;

      // Attach remove method
      this.remove = (callback) => new Promise((resolve) => {
        const id = this.el.getAttribute('data-uuid');
        // Remove ampersand events
        this.off();
    ...
    window.ciscospark.widgetFn[`${name}Widget`].version = process.env.REACT_CISCOSPARK_VERSION;
  }
  setupWidgets();
}
