...
function getCallDataGroup(iWasJoined, noBodyJoined, actorWasMe, actor) {
  let arg, status;
...
export function parseActivityCallData(callData, currentUser) {
  ...
  const actorWasMe = actor.entryUUID === currentUser.id;

  const {arg, status} = isGroupCall
    ? getCallDataGroup(iWasJoined, noBodyJoined, actorWasMe, actor)
