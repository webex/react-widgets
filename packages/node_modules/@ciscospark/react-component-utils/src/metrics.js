const METRIC_NS = `ciscospark-widget-`;
const LOAD_START = `startLoad`;
...
export function sendPerfMetric({key, data}, spark) {
  ...
  return spark.metrics.sendUnstructured(`${METRIC_NS}${key}`, formattedData);
}
...
export function sendTempMetric(key, spark) {
  const metric = getTempMetric(key);
