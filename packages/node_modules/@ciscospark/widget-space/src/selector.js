    ...
    if (conversation.has('locusUrl')) {
      ...
      if (!mediaId &&
        ...
        const matchedCall = media.get('byId').find((call) => {
          if (call && call.instance) {
            const {locus} = call.instance;
            if (locus && locus.host) {
        ...
        if (matchedCall) {
          mediaId = media.getIn(['byLocusUrl', matchedCall.instance.locus.url]);
        }
      }
    }
    ...
    switch (type) {
      ...
      case destinationTypes.PSTN: {
        if (call && call.locus && call.locus.self && call.locus.self) {
          displayName = call.self.displayInfo.primaryDisplayName;
        }
        else {
          displayName = id;
        }
