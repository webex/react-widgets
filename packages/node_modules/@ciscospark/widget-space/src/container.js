...
export class SpaceWidget extends Component {
  ...
  setup(props) { // eslint-disable-line complexity
    ...
    if (sparkInstance && sparkState.get(`authenticated`) && sparkState.get(`registered`)) {
      ...
      if (!widgetStatus.get(`isFetchingSpaceDetails`) && !widgetStatus.get(`isError`) && !spaceDetails) {
        if (spaceId) {
  ...
  render() {
    ...
    if (widgetStatus.get(`isError`)) {
      const errorTitle = spaceDetails.get(`errorTitle`);
      const errorSubtitle = spaceDetails.get(`errorSubtitle`);
      return (
