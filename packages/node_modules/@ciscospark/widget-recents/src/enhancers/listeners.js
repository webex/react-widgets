...
import {API_ACTIVITY_TYPE, API_ACTIVITY_VERB, SPACE_TYPE_ONE_ON_ONE} from '@ciscospark/react-component-utils';
...
function processActivity(activity, space, props) {
  ...
  switch (activity.verb) {
    ...
    case API_ACTIVITY_VERB.LEAVE: {
      ...
      if (activity.object.id === currentUserId) {
...
const currentUserLeft
  = (activity, isCurrentId) => activity.verb === API_ACTIVITY_VERB.LEAVE && activity.object.id === isCurrentId;
...
function handleNewActivity(activity, props) {
  ...
  if (cachedSpace) {
    ...
    // unless the current user left the space
    const getSpace = currentUserLeft(activity, users.get('currentUserId'))
      ? Promise.resolve(space) : props.fetchSpace(sparkInstance, space);
