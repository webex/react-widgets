...
<div style="float: left;">
  ...
  <div style="width: 500px; height: 500px;">
    <main
      class="ciscospark-recents"
      id="ciscospark-widget-direct-data-api"
      data-toggle="ciscospark-recents"
      data-access-token="<%= process.env.CISCOSPARK_ACCESS_TOKEN %>"
      data-enable-space-list-filter="<%= process.env.CISCOSPARK_ENABLE_SPACE_LIST_FILTER %>"
      data-space-type-filter="<%= process.env.CISCOSPARK_SPACE_TYPE_FILTER %>"
    />
  </div>
</div>
...
function createWidget() {
  ...
  widget.recentsWidget({
    ...
    enableSpaceListFilter: '<%= process.env.CISCOSPARK_ENABLE_SPACE_LIST_FILTER %>',
    spaceTypeFilter: '<%= process.env.CISCOSPARK_SPACE_TYPE_FILTER %>'
  });
}
