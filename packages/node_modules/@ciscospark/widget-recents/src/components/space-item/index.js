...
export default function SpaceItem({
  ...
  const actorName = latestActivity.actorName ? latestActivity.actorName.split(` `)[0] : ``;
  ...
  case `update`: {
    if (latestActivity.object.objectType === `locusSessionSummary`) {
      if (latestActivity.object.duration < 2) {
        activityText = formatMessage(messages.unavailable);
      }
    }
    break;
  }
  ...
  const spaceStyle = isDirectSpace ? styles.directSpace : ``;

  return ( // eslint-disable-next-line jsx-a11y/no-static-element-interactions
    <div
      className={classNames(`space-item`, styles.item, spaceStyle)}
      onClick={handleClick}
