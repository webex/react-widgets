...
export default function SpaceItem({ // eslint-disable-line complexity
  avatarUrl,
  formatMessage,
  id,
  isUnread,
  ...
  let activityText, actorName;

  if (!isDecrypting) {
    actorName = latestActivity.actorName;
    activityText = <span>{actorName}<span className={styles.midDot} />{latestActivity.text}</span>;
