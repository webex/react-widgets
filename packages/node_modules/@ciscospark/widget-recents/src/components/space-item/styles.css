.item {
  position: relative;
  display: flex;
  padding: 10px 8px 10px 24px;
  cursor: pointer;
  outline: none;
}

.item:hover {
  background: #efefef;
}

.actions {
  position: absolute;
  top: 10px;
  right: 16px;
  z-index: 10;
  display: none;
}

.item:hover .actions {
  display: block;
}

.call-button {
  background-color: #30d557;
}

.call-button:hover {
  background-color: #2ecc52;
}

.action-button {
  width: 38px;
  height: 38px;
  padding: 11px;
  font-size: 18px;
  color: #fff;
  border-radius: 50%;
}

.avatar-wrapper {
  display: flex;
  align-items: center;
}

.avatar-wrapper > div > div {
  width: 38px;
  height: 38px;
}

.avatar-group {
  border: 2px solid #ccc;
  border-radius: 50%;
}

.avatar-wrapper .avatar-group > div {
  width: 28px;
  height: 28px;
  margin: 3px;
}

.unread-indicator {
  position: absolute;
  top: 46%; /* A bit of a hack but visually looks good */
  left: 8px;
  width: 6px;
  height: 6px;
  background-color: #07c1e4;
  border-radius: 50%;
}

.team-name {
  font-size: 10px;
  text-transform: uppercase;
}

.title {
  overflow: hidden;
  font-size: 14px;
  color: #4f5051;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.title.is-unread {
  font-weight: 300;
}

.meta {
  width: 100%;
  padding: 0 50px 0 8px;
  overflow: hidden;
  flex: 1 1 auto;
}

.last-activity {
  height: 16px;
  overflow: hidden;
  font-size: 12px;
  color: #999;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.timestamp {
  font-size: 10px;
  color: #999;
  white-space: nowrap;
}

.direct-space.has-call-support:hover .timestamp {
  display: none;
}

.mid-dot::after {
  padding: 0 3px;
  content: '\b7';
}

@keyframes placeHolderShimmer {
  0% {
    background-position: -468px 0;
  }

  100% {
    background-position: 468px 0;
  }
}

.isDecrypting .avatar-wrapper,
.isDecrypting .team-name,
.isDecrypting .title,
.isDecrypting .timestamp,
.isDecrypting .last-activity {
  position: relative;
  margin-bottom: 5px;
  line-height: 12px;
  color: transparent !important; /* stylelint-disable-line declaration-no-important */
  background: #efefef;
  background: linear-gradient(to right, #e0e0e0 0%, #ccc 12%, #e0e0e0 30%);
  background-size: 800px 104px;
  animation-duration: 1.5s;
  animation-iteration-count: infinite;
  animation-name: placeHolderShimmer;
  animation-timing-function: linear;
  animation-fill-mode: forwards;
}

.isDecrypting .avatar-group {
  visibility: hidden;
}
