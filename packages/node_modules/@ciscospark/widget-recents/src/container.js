  ...
  handleNewActivity(activity) {
    ...
    case `post`: {
      if (space) {
        ...
        const isSelf = activity.actor.id === user.get(`currentUser`).id;
        ...
        if (!isSelf) {
          handleEvent(eventNames.SPACES_UNREAD, eventActivity);
        }
        handleEvent(eventNames.MESSAGES_CREATED, eventActivity);
      }
      else {
    ...
    case `acknowledge`: {
      if (activity.actor.id === user.get(`currentUser`).id) {
    ...
    if (spacesList && spacesList.count()) {
      return (
        <div className={classNames(`ciscospark-spaces-list-wrapper`, styles.spacesListWrapper)}>
          <SpacesList onClick={this.handleSpaceClick} spaces={spacesList} />
        </div>
      );
    }
