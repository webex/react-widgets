...
export class RecentsWidget extends Component {
      ...
      call.acknowledge()
        .then(() => {
          ...
          const checkForConnected = () => {
            if (call.me === `connected`) {
              call.off(`change:me`, checkForConnected);
              call.off(`inactive`, removeIncoming);
              props.deleteIncomingCall();
            }
          };
          call.on(`change:me`, checkForConnected);
  ...
  render() {
    ...
    const hasWebRTCSupport = widgetStatus.hasWebRTCSupport;
