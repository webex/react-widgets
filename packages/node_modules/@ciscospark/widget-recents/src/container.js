...
export class RecentsWidget extends Component {
  ...
  handleNewActivity(activity) {
    ...
    // Check for notification state update event(s) from a
    // tag which contains "MESSAGE_NOTIFICATIONS_ON", "MESSAGE_NOTIFICATIONS_OFF",
    // "MENTION_NOTIFICATIONS_ON", or "MENTION_NOTIFICATIONS_OFF"
    const {tags} = activity.object;
    const updateNotificationFeatures = tags && tags.size === 1 &&
      (tags[0].inclues[MESSAGE_NOTIFICATIONS_ON]
        || tags[0].includes[MESSAGE_NOTIFICATIONS_OFF]
        || tags[0].includes[MENTION_NOTIFICATIONS_ON]
        || tags[0].includes[MENTION_NOTIFICATIONS_OFF]);
    if (updateNotificationFeatures) {
      // Re-fetch group message notification feature
      props.getFeature(FEATURES_USER, FEATURES_GROUP_MESSAGE_NOTIFICATIONS, sparkInstance).then(() => {
        props.updateWidgetStatus({
          hasFetchedGroupMessageNotificationFeature: true
        });
      ...
      // Re-fetch mention notification feature
      props.getFeature(FEATURES_USER, FEATURES_MENTION_NOTIFICATIONS, sparkInstance).then(() => {
        props.updateWidgetStatus({
          hasFetchedMentionNotificationFeature: true
        });
    ...
    if (spaceId && ['post'].includes(activity.verb)) {
      props.fetchSpace(sparkInstance, spaceId)
        .then((newSpace) => {
  ...
  handleCall(call) {
    ...
    call.acknowledge()
      .then(() => {
        const removeIncoming = () => props.deleteIncomingCall();
  ...
  render() {
    ...
    const {hasFetchedGroupMessageNotificationFeature, hasFetchedMentionNotificationFeature} = widgetStatus;
    ...
    if (spacesListArray &&
      hasFetchedSpaces &&
      hasFetchedGroupMessageNotificationFeature &&
      hasFetchedMentionNotificationFeature) {
      ...
      let filteredSpacesListWithActivityText;
      // if enabled, filter space list based on a keyword filter
      if (enableSpaceListFilter && keywordFilter && keywordFilter.length > 0) {
        filteredSpacesListWithActivityText = spacesListWithActivityText.filter((space) => {
          const {name} = space;
          if (name.toLowerCase().includes(keywordFilter.toLowerCase())) {
            return true;
          }
          return false;
        });
        ...
        <div className={classNames('ciscospark-recents-widget', styles.recentsWidget)}>
          ...
          <div className={classNames('ciscospark-spaces-list-wrapper', styles.spacesListWrapper)}>
            <SpacesList
              ...
              spaces={
                enableSpaceListFilter && keywordFilter && keywordFilter.length > 0
                  ? filteredSpacesListWithActivityText : spacesListWithActivityText
              }
