...
export class RecentsWidget extends Component {
  ...
  handleNewActivity(activity) { // eslint-disable-line complexity
    ...
    case `post`: {
      if (space) {
        ...
        if (!isSelf) {
          handleEvent(eventNames.SPACES_UNREAD, constructRoomsEventData(space.toJS(), activity));
        }
    ...
    case `acknowledge`: {
      if (isSelf && space) {
        ...
        handleEvent(eventNames.SPACES_READ, constructRoomsEventData(space.toJS(), activity));
      }
      break;
    }
    case `add`: {
  ...
  handleSpaceClick(spaceId) {
    ...
    this.handleEvent(eventNames.SPACES_SELECTED, constructRoomsEventData(space.toJS()));
  }
