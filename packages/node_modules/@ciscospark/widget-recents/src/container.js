...
export class RecentsWidget extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    return nextProps.spaces !== this.props.spaces
      ...
      spacesList.forEach((s) => {
        if (s.type === `direct`) {
          if (s.participants.length > 1) {
            ...
            const otherUser = s.participants.find((p) => p.id !== currentUserId);
            if (otherUser) {
              this.fetchAvatar({userId: otherUser.id}, nextProps);
            }
          }
        }
        else {
          this.fetchAvatar({space: spaces.getIn([`items`, s.id]).toJS()}, nextProps);
        }
      });
    }
  }
  ...
  @autobind
  fetchAvatar({space, userId}, nextProps) {
    const {
      avatar,
      sparkInstance
    } = nextProps;
    ...
    return nextProps.fetchAvatar({space, userId}, sparkInstance);
  }
