...
const injectedPropTypes = {
  ...
  checkCurrentCalls: PropTypes.func.isRequired,
...
export class RecentsWidget extends Component {
  ...
  static setup(props) {
    ...
    if (
      ...
      if (widgetStatus.hasFetchedRecentSpaces) {
        // Wait to listen for incoming calls until spaces have loaded
        ...
        if (isWebRTCSupported && !media.getIn(['status', 'isListening'])) {
          RecentsWidget.getCurrentCalls(props);
          props.listenForCalls(sparkInstance);
        }
  ...
  @autobind
  static getCurrentCalls(props) {
    const {
      widgetStatus
    } = props;
    if (widgetStatus.hasFetchedCurrentCalls) {
      return;
    }
    props.updateWidgetStatus({hasFetchedCurrentCalls: true});
    props.checkCurrentCalls(props.sparkInstance);
  }
