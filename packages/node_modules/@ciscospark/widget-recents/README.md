...
| `spaceTypeFilter` | `data-space-type-filter` | (default: `all`)  This filters the list of spaces by type. This can be set to: `direct` for one-on-one conversations, `all` for everything, or `group` for group spaces. |
