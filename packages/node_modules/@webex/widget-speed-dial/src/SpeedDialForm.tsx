  ...
  return (
    ...
    >
        ...
        <Controller
          ...
          render={({ field: { name, onChange } }) => (
            <Select
              ...
              isDisabled={formData.phoneType ? false : true }
              defaultSelectedKey={formData.callType || callTypeOptions[0].id}
              onSelectionChange={(key) => {
                setFormData({ ...formData, callType: key.toString() });
                onChange(key.toString() === callTypeOptions[0].id ? 'handset' : 'video');
              }}
            >
      ...
      <Flex className={sc('footer')} justifyContent="flex-end" xgap="1rem">
        ...
        <ButtonPill type="submit" size={32} disabled={validation} className="btn-margin-left">
          {addText}
        </ButtonPill>
      </Flex>
    </form>
