...
export const SpeedDialItem = ({
  ...
  const handleEnterKeyPressForOutlookContact = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter' || event.keyCode === 13) {
      if (onRemovePress) {
        onRemovePress(item.id);
      }
      ...
      if (contactRef.current) {
        ...
        if(!item?.isOutlookDeleted) {
          contextTriggerRef.current?.setState({
            isVisible: true,
            x: contactRef.current.offsetLeft + 92,
            y: contactRef.current.offsetTop - 30
          });
        }
        // Prevent the default browser context menu
        e.preventDefault();
        if(submenuItemRef.current) {
          console.log('submenuItemRef ', submenuItemRef);
              ...
              <ContextMenuTrigger id={contextMenuId} holdToDisplay={-1}>
                ...
                >
                  <Flex
                    ...
                    role="button"
                    tabIndex={0}
                    onClick={handleRemoveOutlookContactOnClick}
                    onKeyDown={handleEnterKeyPressForOutlookContact}
                    onFocus={handleContextMenuTriggerFocus}
                    onBlur={handleContextMenuTriggerBlur}
                  >
                    {children && (
