...
export const SearchContactsItem = ({
  ...
  const filteredEmails = user.emailAddresses.length > 1 ? user.emailAddresses.filter(email => email.address !== undefined && email.address !== null) : user.emailAddresses;
  const callables = [...user.phoneNumbers, ...filteredEmails];
  const hasNoAddress = callables.length === 1 ? callables.some(callable => callable.address === undefined || callable.address === null) : false;
    ...
    >
      ...
      <ListItemBaseSection position="end">
        <Flex xgap=".5rem" className={sc('actions')}>
          ...
          <CallSelectPopover callables={callables} isVideo ref={videoCallRef} makeCall = {makeCall} label = {label}>
            <ButtonCircle
              color="join"
              size={28}
