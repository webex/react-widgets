import {constructActivity, constructActivityWithContent, isImage, sanitize} from '@webex/redux-module-activity/src/node_modules/@webex/react-component-utils';
...
import {
  ...
  createMessageObject,
  cleanupAfterSubmit
} from '@webex/redux-module-activity/src/helpers';
...
export function submitActivity(conversation, activity, user, spark) {
  return (dispatch) => {
    ...
    if (shareActivity && activity.get('files').size) {
      const inFlightActivity = constructActivityWithContent(conversationJS, activityObject, user, activity.get('files').toArray(), shareActivity);
    ...
    if (activityObject) {
      const inFlightActivity = constructActivity(conversationJS, activityObject, user);
