  ...
  return async () => {
    ...
    switch (toType) {
      ...
      case destinationTypes.SPACEID: {
        // Check if unified meetings is enabled (sdkAdapter.sdk is the raw webex SDK instance)
        const isUnifiedMeetingsEnabled = sdkAdapter.sdk?.meetings?.config?.experimental?.enableUnifiedMeetings;

        if (isUnifiedMeetingsEnabled) {
          try {
            // Call Meetings API to create ad-hoc meeting
            const response = await sdkAdapter.sdk.request({
              method: 'POST',
            ...
            console.log('HELLO', 'Unified meeting created:', meetingDestination);
            destination = meetingDestination;
          }
          catch (error) {
            // eslint-disable-next-line no-console
            console.error('HELLO', 'Failed to create unified meeting:', error);
            // Fall back to original destination if API call fails
            destination = toValue;
          }
        }
        else {
    ...
    props.placeCall(sdkAdapter, {destination, options: callOptions, cleanUp})
      .then((result) => {
        // eslint-disable-next-line no-console
        console.log('HELLO [handleCall] placeCall result:', result);
