...
export const useMakeCall = (crossLaunchToken: string) => {
  const ctx = useContext(AdapterContext);

  const makeCall = useCallback(
    (address: string, isVideo?: boolean) => {
      return makeCrossLaunchCall(address, !!isVideo, crossLaunchToken);
    },
    [makeCrossLaunchCall]
  );
...
export async function makeCrossLaunchCall(address: string, isAudio = false, crossLaunchToken:string) {
  console.debug(
  ...
  callUrl.searchParams.append('jws', crossLaunchToken);
