...
function acknowledgeAdaptiveCardSubmitAction(activityId, parentId) {
  const actionSubmitAcknowledgeObject = {
    cardActionAcknowledgmentId: activityId,
    parentId
  };
 ...
 * @returns {function}
 */
export function handleAdaptiveCardSubmitAction(url, actionInput, parentId, spark) {
  return (dispatch) =>
    spark.internal.conversation.cardAction(
      {url},
      actionInput,
      {id: parentId}
    ).then((activity) =>
      dispatch(acknowledgeAdaptiveCardSubmitAction(activity.id, parentId)))
      .catch(() =>
        dispatch(acknowledgeAdaptiveCardSubmitAction({activityId: null, parentId: null})));
}
