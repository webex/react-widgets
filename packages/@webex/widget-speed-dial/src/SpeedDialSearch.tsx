...
import { IWebexIntContact } from  '@webex/component-adapter-interfaces/dist/cjs/src';
...
const SpeedDialListItem = ({
  ...
  return (
    ...
    >
      ...
      {/*
      add a newly type in component adpater interface then un comment the below one
       <ListItemBaseSection position="start">
        <AvatarNext title={item?.name} size={32} />
      </ListItemBaseSection>
...
export const SpeedDialSearch = ({
  ...
  const [source, filteredContactResponse, isSearching] = useContactSearch(state.searchValue);
  const filterArray = filteredContactResponse.items

  const searchContactsData = {
    outlook: filterArray[source[0]],
    directory: filterArray[source[1]]
  };
                ...
                >
                  ...
                  {searchContactsData?.outlook?.length > 0 && (
                    <>
                      ...
                      {searchContactsData?.outlook.map((item: IWebexIntContact | any) => (
                        <SpeedDialListItem
                          key={`search-item-${item.id}`}
                          className={sc('item')}
                          onPress={handlePress}
