...
export function acceptIncomingCall(
  incomingCall,
  { sdkAdapter, destinationId, cleanUp }
) {
  return (dispatch) => {
    ...
    return Promise.resolve()
      .then(() =>
        meetingsAdapter.datasource.meetings.getMeetingByType(
          "locusUrl",
          incomingCall.locusUrl
        )
      )
      .then((meeting) => {
...
const getMeeting = ({ sdkAdapter: { meetingsAdapter }, destination }) =>
  new Promise((resolve) => {
    ...
    .then(
      tap((meeting) =>
        meetingsAdapter.datasource.meetings.meetingCollection.set(meeting)
      )
    )
    .then((meeting) =>
      meetingsAdapter
...
export function placeCall(sdkAdapter, { destination, cleanUp }) {
  ...
  return (dispatch) =>
    getMeeting({ sdkAdapter, destination })
      .then(
        tap((meeting) =>
