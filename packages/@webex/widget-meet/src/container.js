...
export class MeetWidget extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      ...
      || this.state.escapedAuthentication !== nextState.escapedAuthentication
    );
  }
  ...
  render() {
    ...
    if (displayName && sdkAdapter) {
      ...
      if (isIncoming) {
        const {handleAnswer, handleDecline} = this.props;

        return (
          <div
            className={classNames(
              styles.meetWidgetContainer,
              'meet-widget-container'
            )}
          >
            <IncomingCall
      ...
      // Is the call active?
      if (
        !this.state.escapedAuthentication &&
        meeting &&
        widgetMeet.status.hasInitiatedCall
      ) {
        return (
          <div
            className={classNames(
...
export default compose(connect(getMeetWidgetProps), enhancers)(MeetWidget);
